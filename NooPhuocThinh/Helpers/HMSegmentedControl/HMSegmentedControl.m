...
- (id)initWithSectionTitles:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        if (self.segmentWidthMaxValue == 0) {
            self.segmentWidthMaxValue = 80;
        }
    }
...
- (void)commonInit {
    ...
    self.opaque = NO;
    self.selectionIndicatorColor = COLOR_SELECT_INDICATOR;
...
- (void)drawRect:(CGRect)rect {    
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat stringWidthOldIndex = 10;
            if (idx > 0) {
                stringWidthOldIndex = [self getContentWidthByIndex:idx];
            }
            
            CGRect rect = CGRectMake(stringWidthOldIndex, y, MAX(stringWidth, self.segmentWidthMaxValue), stringHeight);
...
-(CGFloat) getContentWidthByIndex:(int) index {
    
    CGFloat stringWidthOldIndex = 10;
    
    for (int i=0; i<index; i++) {
        stringWidthOldIndex += MAX(roundf([[self.sectionTitles objectAtIndex:i] sizeWithFont:self.font].width), self.segmentWidthMaxValue) + 15;
    }
    
    return stringWidthOldIndex;
}
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        ...
        sectionWidth = MAX(stringWidth, self.segmentWidthMaxValue);
    } else if (self.type == HMSegmentedControlTypeImages) {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleTextWidthStripe && sectionWidth <= self.segmentWidth) {
        CGFloat stringWidthOldIndex = [self getContentWidthByIndex:self.selectedSegmentIndex];
        
        CGFloat stringWidth = roundf([[self.sectionTitles objectAtIndex:self.selectedSegmentIndex] sizeWithFont:self.font].width);
        
        return CGRectMake(stringWidthOldIndex, indicatorYOffset, MAX(stringWidth, self.segmentWidthMaxValue), self.selectionIndicatorHeight);
    } else {
...
- (CGRect)frameForFillerSelectionIndicator {
    ...
    return CGRectMake(stringWidthOldIndex, 0, MAX(stringWidth, self.segmentWidthMaxValue), CGRectGetHeight(self.frame));
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        NSInteger segment = [self checkPageScrollByLocationX:touchLocation.x + self.scrollView.contentOffset.x];
...
-(NSInteger) checkPageScrollByLocationX:(CGFloat) locationX {
    
    NSInteger segmentPage = 0;
    for (int i=0; i<self.sectionTitles.count; i++) {
        
        CGFloat stringWidthOldIndex = [self getContentWidthByIndex:i];
        CGFloat stringWidth = roundf([[self.sectionTitles objectAtIndex:i] sizeWithFont:self.font].width);
        
        if (stringWidthOldIndex + MAX(stringWidth, self.segmentWidthMaxValue) > locationX) {
...
- (CGFloat)totalSegmentedControlWidth {
    if (self.type == HMSegmentedControlTypeText) {
        ...
        for (int i=0; i<self.sectionTitles.count; i++) {
            ...
            totalWidth += MAX(roundf([titleString sizeWithFont:self.font].width), self.segmentWidthMaxValue) + 15;
        }
...
- (void)scrollToSelectedSegmentIndex {
    ...
    CGRect rectForSelectedIndex = CGRectMake(stringWidthOldIndex,
                                             0,
                                             MAX(stringWidth, self.segmentWidthMaxValue),
                                             self.frame.size.height);
    
    CGFloat selectedSegmentOffset = (CGRectGetWidth(self.frame) / 2) - (MAX(stringWidth, self.segmentWidthMaxValue) / 2);
