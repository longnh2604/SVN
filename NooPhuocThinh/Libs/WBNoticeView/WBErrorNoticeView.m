...
- (void)showNoticeWithOffset:(CGFloat) offset
{
    NSUserDefaults *defaultNotice = [NSUserDefaults standardUserDefaults];
    [defaultNotice setBool:YES forKey:@"ShowNoticeNooPhuocThinh"];
...
//    TFLog(@"titleLabelFrame: %@", NSStringFromCGRect(self.titleLabel.frame));
...
//    self.titleLabel.layer.borderColor = [UIColor greenColor].CGColor;
    ...
    [self displayNoticeWithOffset:offset];
}
...
-(BOOL) isShowNotice
{
    if (self.gradientView.frame.origin.y == 43)
        return YES;
    else
        return NO;
}

//-- check show Notice on WebView
-(BOOL) isShowNoticeWebView
{
    if (self.gradientView.frame.origin.y == -1)
        return YES;
    else
        return NO;
}
