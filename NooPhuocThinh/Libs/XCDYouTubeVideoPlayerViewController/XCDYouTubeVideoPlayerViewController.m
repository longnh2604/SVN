...
- (void) connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    NSString *filePath = [Utility getVideoFilePathWithCategoryId:self.videoCategoryId withNodeId:self.videoNodeId];
    ...
    if (isShowDownLoad == YES) {
        ...
        NSString *urlVideodefault = [defaultVideoPath valueForKey:@"VideoPathDefault"];
        
        NSError *error = nil;
        NSURL *videoURL = [self videoURLWithData:self.connectionData error:&error];
        
        if (!videoURL) //-- if error link youtube get link server
            videoURL = [NSURL URLWithString:urlVideodefault];
