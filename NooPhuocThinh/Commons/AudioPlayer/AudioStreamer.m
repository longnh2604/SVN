...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
		...
		{
			NSLog(@"long: %@", [AudioStreamer stringForErrorCode:anErrorCode]);
		}
...
- (BOOL)openReadStream
{
	...
	@synchronized(self)
	{
        ...
        if (fixedLength) {
			...
			if (!stream)
                NSLog(@"Could not open file stream for: %@", [url absoluteString] );
            ...
            {
                CFNumberRef pos_property = CFNumberCreate(NULL, kCFNumberSInt32Type, &seekByteOffset);
                CFReadStreamSetProperty(stream, kCFStreamPropertyFileCurrentOffset,pos_property);
                discontinuous = YES;
            }
...
- (double)duration
{
	...
	//longnh: fix bug tinh sai duration
    double d = (fileLength - dataOffset) / (calculatedBitRate * 0.125);
//    if (d < 0) {
//        d = 0;
//    }
	return d;
...
- (void)setVolume:(float)Level
{
    ...
    if (errorMsg) {
        NSLog(@"AudioQueueSetParameter returned %d when setting the volume.", (int)errorMsg);
    }
...
- (void)handleReadFromStream:(CFReadStreamRef)aStream eventType:(CFStreamEventType)eventType
{
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		}else if (fixedLength == YES) {
            
            if (seekByteOffset == 0) {
                
                NSData *attributes = [NSData dataWithContentsOfURL:url];
                fileLength = [attributes length];
            }
        }
